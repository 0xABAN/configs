...
3. Choose one execution path after main-agent scoping:
   ...
   - One independent track that benefits from isolated context: one subagent is allowed.
   - 2+ independent tracks: launch all relevant subagents in the same response.
...
- Do **not** write design/spec files (e.g. under `docs/superpowers/specs/`) or commit them unless the user explicitly asks for a written design doc.
- Conversational design approval is enough. Prefer plan or implement next, not a disk artifact.
...
Use the number of subagents that fits the scoped work. One subagent is allowed when a single independent track benefits from isolated context; for multiple independent tracks, launch them together.
...
A subagent call blocks the main agent, so main agent + 1 subagent is sequential isolation, not parallelism. When using multiple subagents, launch them as a batch in the same response.
