...
def smoke(sdk: Path, launcher: Path, config: Path, home: Path, output: Path) -> None:
    ...
    Other personal extensions are deliberately excluded: intercom/MCP/subagents
    can contact live peers or services. Native package suites cover their renderers.
    """
    ...
    settings.update(quietStartup=True, lastChangelogVersion="0.85.1")
...
import { fauxProvider, fauxAssistantMessage } from ''' + json.dumps(str(faux)) + ''';
export default function (pi) {
  const root = process.env.PI_CLI_SMOKE_OUTPUT;
  const faux = fauxProvider();
  pi.registerProvider(faux.provider);
  faux.setResponses([fauxAssistantMessage("OFFLINE_CUSTOM_HOST_RESPONSE")]);
...
''')
    ...
    for mode in ("regular", "fullscreen"):
        ...
        subprocess.run(["tmux", "new-session", "-d", "-s", target, "-x", "120", "-y", "36", "-c", "/tmp",
        ...
        try:
            ...
            print(f"{mode}: actual {launcher} rendered headers, cream separator, 2-column outer inset, Powerline and faux response")
        finally:
